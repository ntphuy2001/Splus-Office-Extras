...
{
    internal class GGTranslateService: ITranslationService
    {
        private string _token;

        public GGTranslateService(string token)
        {
            _token = token;
        }
        ...
        public List<string> Translate(List<Range> listResources, string sourceLanguage, string targetLanguage)
        {
            ...
            for (int i = 0; i < listResources.Count; i++)
            {
                ...
                listResult.Add($"GGTranslate translate from {sourceLanguage} to {targetLanguage}: {resource.Text.TrimEnd()}");
            }
